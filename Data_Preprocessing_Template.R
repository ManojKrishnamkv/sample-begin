...
Salary_dept_plot <- ggplot(Salary_avg, aes(x = Avg , y = JobRole, fill = JobRole)) +
  geom_bar(stat = "identity") +
  labs(title = "Average Monthly Income by Job Role", x = "Average", y = "JobRole") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Display the plot
print(Salary_dept_plot)
